// For Game Script Native Swift from Feb 19, 2022.

//fn NumberArray() {
//	return [];
//}

func meshConvertToIndexMesh(_ mesh: Number) { // Number
	meshClearIndices(mesh);
	for i in stride(from: 0, to: meshGetVerticesCount(mesh) / 3, by: 1) {
...
if (getCurrentFrame() == 0) {
	...
	append(&vertex, 0);
	append(&vertex, 0);
	for i in stride(from: 0, to: verticesComponentsCount, by: 1) {
		pointerSetSubstring(stringNumber, 0, verticesString, i*10, 10);
		vertex[Int(i) % 3] = interpretStringToFloat(pointerGetString(stringNumber));
		if (Int(i) % 3 == 2) {
	...
	do {
		...
		for i in stride(from: 0, to: verticesComponentsCount / 9, by: 1) {
...
if (isRerun() == true) {
	if (pointerIsNull(globalArrayPersistentGetPointer("devicep")) == false) {
		...
		if (pointerIsNull(device) == false) {
			for i in stride(from: 0, to: len(handles), by: 1) {
				if (pointerIsNull(handles[Int(i)]) == false) {
					ertDecRef(device, handles[Int(i)]);
				}
			}
...
if (pointerGetUnsignedInteger(currentMode, 0) == 1) {
	...
	for y in stride(from: 900 - 1, to: -1, by: -1) {
